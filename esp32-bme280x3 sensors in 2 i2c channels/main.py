...
2 I2C channels, pins defined in runtimeconfig. Asynchoronous code.

First BME280 is set to Pin(22) = SCL and Pin(21) = SDA (I2C1)
Second BME280 is set to Pin(32) = SCL and Pin(33) = SDA AND SDO is OPEN or GND (I2C2)
Third BME280 is set to Pin(32) = SCL and Pin(33) = SDA AND SDO is connected to +3.3V! (I2C2)
If you need to change pins, do that in the parameters.py

bmes = First BME280 which uses default address 0x76
bmet = Second BME280 which uses default address 0x76
bmeu = Third BME280 which uses address 0x77 because SDO is connected to VCC
Each sensor has MQTT messages defined in runtimeconfig.json

MQTT messages could be sent to Influxdb and from influxdb to Grafana.

First version: 5.10.2022: Jari Hiltunen & Tarmo Hiltunen
